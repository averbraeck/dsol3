 ...
 */
public class InputParameterMap extends AbstractInputParameter<SortedMap<String, InputParameter<?>>>
{
    /** */
    ...
    public void remove(final String key) throws InputParameterException
    {
        ...
        if (!(parent instanceof InputParameterMap))
        {
            throw new InputParameterException(
                    "Key " + key + " in InputParameterMap " + getKey() + " is not a leaf, but also not an InputParameterMap");
        }
        ((InputParameterMap) parent).remove(key.substring(key.indexOf('.') + 1));
    }
    ...
    public InputParameter<?> get(final String key) throws InputParameterException
    {
        ...
        return ((InputParameterMap) parent).get(key.substring(key.indexOf('.') + 1));
    }

    /** {@inheritDoc} */
    @Override
    public InputParameterMap clone() throws CloneNotSupportedException
    {
        InputParameterMap ipm = (InputParameterMap) super.clone();
