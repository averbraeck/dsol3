...
public final class EventListPerformanceTest
{
    ...
    private EventListPerformanceTest()
    {
        ...
        EventListInterface[] testLists = {new RedBlackTree<SimTimeDouble>(), new EventListPriorityQueue<SimTimeDouble>()};
        String[] line = new String[2 * testLists.length + 1];
        results.add(line);
        line[0] = "nr_events";
        int eventlistCount = 1;
        for (EventListInterface<SimTimeDouble> elist : testLists)
        {
        ...
        for (int max : new int[] {100, 1000, 10000, 100_000, 1_000_000})
        {
            ...
            for (EventListInterface<SimTimeDouble> elist : testLists)
            {
                ...
                for (int i = 0; i < max; i++)
                {
                    elist.add(new SimEvent.TimeDouble(dist.draw(), this, test, "mArg1", new Object[] {12.7}));
                }
