/*
 * @(#)DistWeibull.java Apr 3, 2003 Copyright (c) 2002-2005 Delft University of
...
public class DistWeibull extends DistContinuous
{
     ...
     * @param beta (scale)
     */
    public DistWeibull(final StreamInterface stream, final double alpha,
            final double beta)
    {
        super(stream);
        ...
        } else
        {
            throw new IllegalArgumentException(
                    "Error Weibull - alpha <= 0.0 or beta <= 0.0");
        }
    }
    ...
    {
        return this.beta
                * Math.pow(-Math.log(this.stream.nextDouble()),
                        1.0d / this.alpha);
    }
    ...
    public double probDensity(final double observation)
    {
        if (observation > 0)
        {
            return this.alpha
                    ...
                    * Math.pow(observation, this.alpha - 1)
                    * Math.exp(Math.pow(-1 * (observation / this.beta),
                            this.alpha));
        }
        return 0.0;
    }
