...
public class DistLogNormal extends DistNormal
{
     ...
     */
    public DistLogNormal(final StreamInterface stream, final double mu,
            final double sigma)
    {
        super(stream);
        this.mu = mu;
    ...
    public double probDensity(final double observation)
    {
        ...
        {
            return 1
                    / (observation * Math.sqrt(2 * Math.PI
                            * Math.pow(this.sigma, 2)))
                    * Math.exp(-1
                            * Math.pow(Math.log(observation) - this.mu, 2)
