/*
 * @(#) RemoteContextInterface.java Apr 7, 2004 Copyright (c) 2002-2005 Delft
...
{
    /**
     * Constant defining the first part of properties wrapped by our remote
     * construction.
     */
     ...
     * identified by a target and scope changes. The event source of those
     * events is this context. See the class description for a discussion on
     * event source and target. See the descriptions of the constants
     * <tt>OBJECT_SCOPE</tt>,<tt>ONELEVEL_SCOPE</tt>, and
     * <tt>SUBTREE_SCOPE</tt> to see how <tt>scope</tt> affects the
     * registration.
     * <p>
     * <tt>target</tt> needs to name a context only when <tt>scope</tt> is
     * <tt>ONELEVEL_SCOPE</tt>.<tt>target</tt> may name a non-context if
     * <tt>scope</tt> is either <tt>OBJECT_SCOPE</tt> or
     * <tt>SUBTREE_SCOPE</tt>. Using <tt>SUBTREE_SCOPE</tt> for a
     * non-context might be useful, for example, if the caller does not know in
     * advance whether <tt>target</tt> is a context and just wants to register
     * interest in the (possibly degenerate subtree) rooted at <tt>target</tt>.
     * <p>
     ...
     * <tt>EventContext</tt> concurrently. See the <a
     * href=package-summary.html#THREADING>package description </a> for more
     * information on threading issues.
     * 
     * @param target A nonnull name to be resolved relative to this context.
     * @param scope One of <tt>OBJECT_SCOPE</tt>,<tt>ONELEVEL_SCOPE</tt>,
     *        or <tt>SUBTREE_SCOPE</tt>.
    ...
    public void addNamingListener(Name target, int scope,
            RemoteContextListenerInterface l) throws RemoteException,
            NamingException;

    /**
     * Adds a listener for receiving naming events fired when the object named
     * by the string target name and scope changes. See the overload that
     * accepts a <tt>Name</tt> for details.
     * 
     * @param target The nonnull string name of the object resolved relative to
     *        this context.
     * @param scope One of <tt>OBJECT_SCOPE</tt>,<tt>ONELEVEL_SCOPE</tt>,
     *        or <tt>SUBTREE_SCOPE</tt>.
     * @param l The nonnull listener.
    ...
    public void addNamingListener(String target, int scope,
            RemoteContextListenerInterface l) throws NamingException,
            RemoteException;
     ...
     * 
     * @param l The nonnull listener.
     * @exception NamingException If a problem was encountered while removing
     *            the listener.
     * @exception RemoteException on network exception
     */
     ...
     *         exist.
     * @exception NamingException If the context's behavior in this regard
