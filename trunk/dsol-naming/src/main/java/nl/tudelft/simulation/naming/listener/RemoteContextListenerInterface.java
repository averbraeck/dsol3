...
public interface RemoteContextListenerInterface extends Remote
{
     ...
     * <p>
     * The binding of the newly added object can be obtained using <code>evt.getNewBinding()</code>.
     ...
     * <p>
     * The binding of the newly removed object can be obtained using <code>evt.getOldBinding()</code>.
     * @param evt NamingEvent; The nonnull event.
     ...
     * <p>
     * The binding of the renamed object can be obtained using <code>evt.getNewBinding()</code>. Its old binding (before the rename)
     * can be obtained using <code>evt.getOldBinding()</code>. One of these may be null if the old/new binding was outside the scope
     * in which the listener has registered interest.
     * @param evt NamingEvent; The nonnull event.
     ...
     * <p>
     * The binding of the changed object can be obtained using <code>evt.getNewBinding()</code>. Its old binding (before the change)
     * can be obtained using <code>evt.getOldBinding()</code>.
    ...
    /**
     * Called when a naming exception is thrown while attempting to fire a <code>NamingEvent</code>.
