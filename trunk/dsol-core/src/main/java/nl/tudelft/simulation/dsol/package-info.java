 ...
 * gathered and presented during and after the simulation run for providing insight into the most important performance
 * indicators. In most cases, animation and visualization are added to provide insight and understanding into the system
 * modeled during the run. Currently we see a lot of interest in object-oriented simulation environments, where
 * real-world objects can be captured in the simulation environment as object representations. Many of the simulation
 * applications in logistics, business process engineering, and information systems modeling deal with distributed
 * systems with distributed information sources. Therefore, it seems logical to strive for a simulation environment that
 * truly captures the distribution, and that does not reduce the effects of the distributed nature to an artificial time
 * delay. Most current discrete event simulation packages are, however, not designed to cooperate in a distributed
 * computing environment. There are several reasons why it would be interesting to have a simulation environment that
 * has a truly distributed nature.
 * <P>
 ...
 * inter-business component communication) have not yet matured, a simulation architecture can at least be expected to
 * be extendible with such interaction mechanisms. This implies for example separating the core simulation service from
 * the model components that use it. In this way the simulation will become a central service in a distributed, dynamic
 * network where the exact capabilities and identity of the acting and reacting realtime components remains hidden for
 * the simulation core. In the most extreme peer to peer case, objects will use the simulation service just as another
 * way of scheduling their interactions.
 * <P>
 * Another issue is the lack of separation between visualization and simulation in current simulation environments.
 * First, the place where the simulation can best be run (e.g. on a fast server) is not always the place where we want
 * to view the simulation (e.g. in a Web browser at a client's location). Second, due to the global and complex nature
 * of current business systems or logistical systems, the number of stakeholders involved in these systems is large.
 * Moreover, these stakeholders do not reside in one location, but are spread all over the world. In order to provide
 * them with a customized view on a simulation representing their business, there is a need to decouple visualization
 * from the simulation and to deliver this visualization in an interactive way to distributed clients (e.g. a web
 * browser or pda). When several stakeholders are in this way concurrently interacting with the simulation, distributed
 * simulation can become distributed gaming.
 * <p>
 ...
 * automated form, such as in datawarehouses, in ERP databases, or in legacy systems. Since these systems probably
 * contain the most consistent, accurate and actual information of the business system, it seems logical to use that
 * information to specify a simulation. This implies the need to enable a simulation to interact with (distributed)
 * information systems, perhaps even during the run of the simulation. In this way generators, statistical objects and
 * even some of the transactional events could be automatically instantiated and initialized from underlying data
 * sources.
 * </p>
 * <p>
 ...
 * built. As indicated, current implementations of discrete event simulation software are not sufficiently capable to
 * support this distributed cooperation. The next section will present some background on existing object-oriented
 * simulation packages. Section 3 will point out the approach that we have taken to develop a new distributed simulation
 * and representation architecture called <I>DSOL</I>, which stands for <I>Distributed Simulation Object Library</I>. In
 ...
 * </UL>
 * Integrated simulation environments are not suited for a distributed deployment since visualization is strictly bound
 * to the same runtime environment as the simulation. Further characteristics are single-user, single-computer,
 * single-thread implementations of the environment. Most commercially available simulation packages fall in this
 * category.
 * <p>
 ...
 * Silk, JavaSim, SimJava and DESMO-J (Garrido 2001). An example of the second category is the simulation package that
 * can present its results during and after the run in html-format to distant viewers on the web.
 * </p>
 * <p>
 * Fully-distributed simulation environments consist of distributed simulation and visualization services. They provide
 * integration with distributed information systems and endless variations in configuration. Figure 1 is an illustration
 * of this architecture.
 * </p>
 * <p>
 * Since the Java programming language has strong support for distributed interconnectivity, it is reasonable to assume
 * that current Java-based simulation languages characterized as semi-distributed can easily be used to deploy
 ...
 * multithreaded execution is an essential aspect of the implementation for this process-oriented modeling capability.
 * The implication of this multithreaded approach for the distributed goal are:
 * <UL>
 * <LI>Starvation occurs when one or more threads in a program are blocked from gaining access to a resource and thus
 * cannot make progress. Deadlock is the ultimate form of starvation; it occurs when two or more threads are waiting on
 * a condition that cannot be satisfied. Deadlock most often occurs when two (or more) threads are each waiting for each
 ...
 * <LI>Inflexibility of processing intelligence. In distributed networks, remote method invocation in conjunction with
 * dynamic class downloading has facilitated dynamic transfer of business and routing logic between the distributed
 * nodes in a network (Sing 2000). The process-oriented modeling capability prevents this runtime learning mechanism
 * because processing and routing intelligence is not stored in the stations or nodes but in the entities flowing
 ...
 * <LI>Since component libraries such as Enterprise Java Beans that are used in actual information systems are not built
 * upon a multithreading model, the interchange of libraries between these realtime information systems and simulation
 * models is limited.</LI>
 ...
 * schedule them by a constructing a simulation event. Such an event encompasses the execution time, a (remote) pointer
 * to the object on which the method is intended to be invoked, the method itself and the arguments for the method to be
 * invoked. This mechanism is referred to as scheduling method invocation.
 * </p>
 * <p>
 * Running a simulation model merely implies executing the time-sorted simulation events. The absolute simulation time
 * therefore becomes the execution time of the last executed simulation event. Figure 2 illustrates this worldview by
 * focusing on the way simulation events are stored (1) in the eventlist, ordered on time, retrieved (2) and finally
 * executed (3) on the objects referred to in the simulation event. Examples of these simulation languages include
 * Simkit (Buss 2001).
 * </p>
 * <p>
 * The advantages of a DES implementation are its single threaded approach, its increased flexibility (Buss 2001), and
 * the lack of requirements on objects involved in the simulation.
 * </p>
 ...
 * IMPLEMENTED AS A REMOTE NETWORK SERVICE</i> The second service is a system representation service described in
 * subsection <i>SYSTEM REPRESENTATION IMPLEMENTED AS A REMOTE NETWORK SERVICE</i>
 * </p>
 * <p>
 ...
 * <p>
 * As stated the disadvantage of DES-based simulation languages is the increased complexity in model construction. Most
 * other implementations ease this complexity by one or more pragmatic design patterns. Most of these patterns conflict
 * though with a truly distributed architecture.
 * </p>
 * <p>
 * Examples of these patterns are a static eventlist, relative time scheduling, non-serializable reflection classes, the
 * lack of overloading primitive attributes, and the requirement to extend from an abstract simCoreObject which prevents
 * interchange of simObjects and non-simulated components.
 * </p>
 * <p>
 ...
 * aspects of the system. This is especially true when large-scale, complex systems are considered. This implies a need
 * to 'browse' the system (Spence 2001).
 * </p>
 * <p>
 * Secondly, a user may interact with a model in several modes. We characterize the interaction as <I>passive</I>, if
 * the user does not interact with the model during a run. In this mode the user is still able to dynamically define his
 * area of interest (e.g. to 'browse' the system representation). However, these actions will not change the outcomes of
 * the simulation run. The interaction is characterized as <I>active</I>, if the user is able to interact with the model
 * during a run. The user's actions will then influence the actual outcomes of the simulation run. The distinction
 * between passive and active model interaction has consequences for the relation between representation and simulation.
 * With passive interaction, the representation data only needs to be generated once by a simulation run. After that,
 * many users could concurrently iterate over the generated representation data. Given the composition of the user group
 * and the ICT resources available, the representation data could be distributed among the users in several ways, given
 ...
 * separating the system representation from the simulation components, various combinations between real systems (e.g.
 * information system) and simulated systems become possible. The representation presented to the user may be generated
 * in each of them.
 * </p>
 * <p>
 * Finally, the remote aspect of system representation poses some challenges of its own. The devices and networks used
 * by the various users may be as diverse as the users themselves. They may range from smart mobile phones up to
 ...
 * available. Such adaptions are possible, since state change data can be represented in various ways. Information on
 * the movement of a transport vehicle might for example be represented by a text message, but could also be represented
 * in a 3D virtual world. Obviously, these representation modes pose different requirements on the communication and
 ...
 * simulate an actual, real-time supply chain. A screenshot is shown in Figure 4. In the remainder of the section, the
 ...
 * <LI><I>Representation components</I> enable clients to retrieve various <I>event handlers</I>, thus implementing the
 * representation service. Event handlers can transform incoming state change updates into (visual) representations and
 * may provide interaction with an underlying model component.</LI>
 ...
 * event-list does not need to reside client-side. Several clients could share one server-side event list. It may be
 * noted that this event list contains state update events, in contrast with the simulation event list, that contains
 * events triggering an action. Since they both contain time-ordered events they are similar components. The client-side
 * dynamic representation process is then controlled by an EventController component, that iterates through the
 * event-list usually in a time-ordered manner. Such a controller object may itself be controlled by some kind of timer,
 * that updates the representation time of the system. By changing the properties of this timer, the user is able to
 * control the speed and period of the system representation.
 * </p>
 * <p>
 * The process described thus far provides a mechanism to process a state-update event on the correct representation
 * time. However, since these events basically only contain the state-change information itself, additional components
 * are needed to transform this information in an actual, dynamic representation. This is illustrated on the right side
 * of Figure 5, by the interaction between the client-side visualization manager and the representation components in
 * the network. Initially, the client has no event handlers available. When an event needs to be processed for which no
 * event handler is yet available, the client tries to retrieve this event handler from the representation components,
 * based on:
 * </p>
 * <UL>
 ...
 * at <a href="http://www.simulation.tudelft.nl/dsol/dsol/index.html">http://www.simulation.tudelft.nl/dsol/dsol/
