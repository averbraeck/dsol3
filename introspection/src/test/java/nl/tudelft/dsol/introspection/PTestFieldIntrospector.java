    ...
    public static void main(final String[] args)
    {
        try
        {
            Logger.info(PTestFieldIntrospector.class, "main",
                    "Running field introspector test");
            Property[] props = (new FieldIntrospector())
                    .getProperties(new SubTestBean2());
            for (int i = 0; i < props.length; i++)
            {
                Logger.info(PTestFieldIntrospector.class, "main", "Prop name: "
                        + props[i].getName());
                Logger.info(PTestFieldIntrospector.class, "main",
                        "Prop class: " + props[i].getType());
                Logger.info(PTestFieldIntrospector.class, "main",
                        "Prop value: " + props[i].getValue());
                Logger.info(PTestFieldIntrospector.class, "main",
                        "Setting Possible? ");
                props[i].setValue("TEST");
                Logger.info(PTestFieldIntrospector.class, "main",
                        "If so, 'TEST' should be retrieved: "
                                + props[i].getValue());
            }
